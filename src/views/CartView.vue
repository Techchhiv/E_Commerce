    ...
    <div class="font-Playfair h-fit flex pb-[100px] px-[100px] gap-[65px] justify-center max-xl:flex-col max-xl:items-center">
        <div class="w-[680px] min-h-[497px] h-fit px-[35px] pt-[52px] pb-[20px] relative border border-black flex-col justify-start items-start inline-flex">
            ...
            <div class="flex gap-[37px] my-[40px]">
                <button v-on:click="updateSelectedOption('0.99')" :class="{'scale-110 border-blue-950 ':selectedOption=='0.99'}" class="hover:border-black flex-col border px-[18px] py-[15px] border-stone-300 cursor-pointer hover:scale-110 transition ease-in-out w-[178px] h-[95px] rounded-[10px]">
                ...
                <button v-on:click="updateSelectedOption('1.99')" :class="{'scale-110 border-blue-950 ':selectedOption=='1.99'}" class="hover:border-black border px-[18px] py-[15px] border-stone-300 cursor-pointer hover:scale-110 transition ease-in-out w-[178px] h-[95px] rounded-[10px]">
                ...
                <button v-on:click="updateSelectedOption('0.00')" :class="{'scale-110 border-blue-950 ':selectedOption=='0.00'}" class="hover:border-black border px-[18px] py-[15px] border-stone-300 cursor-pointer hover:scale-110 transition ease-in-out w-[178px] h-[95px] rounded-[10px]">
            ...
            <div class="no-scroll flex-col w-full max-h-[470px] overflow-y-auto">
                <div v-if="product" class="flex mb-10" v-for="(p, index) in product">
                    <img class="w-20 h-[123px] border border-black" :src="`/src/assets/image/${category[index]}/${p.image}`" />
                    <div class="ml-[50px] w-full flex flex-col justify-between">
                        <div class="text-black text-xl font-medium font-['Lato']">{{ p.name }}</div>
                        <div class="text-[13px] line-clamp-2 mb-2 h-[35px]">{{ p.description }}</div>
                        <div class="flex gap-2 -[444px] text-zinc-500 text-sm font-normal font-['Lato']">
                            <div class="flex gap-2">
                                <p>${{ p.price }}</p>
                                <p v-if="p.discount!=0">(-{{ p.discount }}%)</p>
                            </div>
                            <p class="ml-[0px] font-bold text-black">${{ (p.price*(1-p.discount/100)).toFixed(2) }}</p>
                        </div>
                        <div class="flex gap-10 justify-between w-full">
                            ...
                            <div class="flex font-bold">
                                ...
                                <p>{{ (p.price*(1-p.discount/100)).toFixed(2)*p.pivot.quantity }}</p>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="w-[370px] px-[48px] py-[48px] h-fit border border-black flex-col justify-center flex">
            ...
            <div class=" flex  mb-[8px] justify-between text-black text-lg font-normal font-['Lato']">
                ...
                <p>${{  parseFloat(getTotal())+parseFloat(selectedOption) }}</p>
            </div>
            ...
            <div class="text-center ">
                <button class="w-[262px] h-[60px] bg-stone-400 rounded-[20px] text-white text-xl font-normal font-['Lato']">Proceed to purchase</button>
            </div>
        </div>
